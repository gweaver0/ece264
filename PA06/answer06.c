 ...
 * Good luck.
 */
struct Image *endexit(FILE *fp, int *headers)
{
  fclose(fp);
  free(headers);
  return NULL;
}
struct Image *endexit2(FILE *fp, int *headers, char *comment)
{
  fclose(fp);
  free(comment);
  free(headers);
...
struct Image *endexit3(FILE *fp, int *headers, char *comment, uint8_t *pixels)
{
  fclose(fp);
  free(comment);
  free(headers);
...
struct Image * loadImage(const char* filename)
{
  ...
  if(headers[1]<=0||headers[2]<=0)
    return endexit(fp, headers); //step 4
  ...
  img -> comment = malloc(sizeof(char)*headers[3]);
  strcpy(img -> comment, comment);
  img -> data = malloc(sizeof(uint8_t)*size);
  for(i =0; i<size;i++)
    img -> data[i] = pixels[i];
  fclose(fp);
  free(pixels);
